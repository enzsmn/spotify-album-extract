...
const CLEAN_URL = process.env.VUE_APP_URL.replace("http://", "").replace(
  "https://",
  ""
);
    ...
    const cb = () => {
      axios
        ...
        .then(res => {
          ...
          const tracksWithAlbums = res.data.items.filter(
...
const createPlaylist = name => {
  return axios
    .post(`${SPOTIFY_API}/users/${lscache.get("spotify_user_id")}/playlists`, {
      name: `${name} – Albums`,
      public: false,
      description: `Albums extracted from “${name}” on ${moment().format(
        "YYYY-MM-DD"
      )} with ${CLEAN_URL}`
    })
    .then(res => {
...
const addTracksToPlaylist = (playlistId, tracks) => {
  ...
  return new Promise(resolve => {
    ...
    chunks.forEach(c => {
      promises.push(
        axios
          .post(`${SPOTIFY_API}/playlists/${playlistId}/tracks`, {
            uris: c
          })
          .then(() => {
            count += c.length;
