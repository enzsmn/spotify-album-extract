<template>
  <div class="page-playlists">
    <div class="buttons">
      <h1>
        <a href="/">Spotify <span>album extract</span></a>
      </h1>
      <b-button
    ...
    <div class="panels">
      <div
        ...
        class="panel playlists"
        :class="{ 'playlist-selected': selectedPlaylist }"
      >
      ...
      <div v-if="selectedPlaylist && albums.length > 0" class="panel albums">
        ...
        <div class="select-buttons">
          <b-button
            type="is-primary is-rounded is-outlined is-small"
            @click="selectedAllAlbums"
            >Select all</b-button
          >
          <b-button
            type="is-primary is-rounded is-outlined is-small"
            @click="deselectedAllAlbums"
        ...
        >
          ...
          <h3 class="item-title">
            ...
            <span class="small muted"
              >{{ album.total_tracks }}
              {{ album.total_tracks > 1 ? "tracks" : "track" }}</span
            >
          </h3>
...
export default {
  ...
  computed: {
    filteredPlaylists() {
      const filterCleaned =
        this.filter && this.filter.trim() ? this.filter.trim() : null;
      if (filterCleaned) {
        return this.playlists.filter(p =>
          p.name.toLowerCase().includes(filterCleaned.toLowerCase())
        );
      }
  ...
  methods: {
    setupAxios() {
      axios.defaults.headers.common["Authorization"] =
        "Bearer " + lscache.get("spotify_auth_code");
    ...
    save() {
      ...
      SpotifyService.getAlbumsTracks(this.selectedAlbums).then(
        tracksChunked => {
          SpotifyService.createPlaylist(this.selectedPlaylist.name).then(
            playlistId => {
